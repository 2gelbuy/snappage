...
const modeTabs = document.querySelectorAll<HTMLButtonElement>('.mode-tab:not([disabled])');
...
const actionAgain = document.getElementById('action-again') as HTMLButtonElement;
...
async function loadSettings() {
  try {
    const data = await chrome.storage.local.get(['mode', 'format', 'quality', 'tipDismissed']);
    if (data.mode === 'full-page' || data.mode === 'visible-area' || data.mode === 'selection') {
...
function setSuccess(filename: string) {
  ...
  statusEl.innerHTML = `
    ...
    <span class="status-text">Captured! Editor opened in new tab.</span>
  `;
  quickActions.style.display = 'none';
...
async function trackCapture() {
  try {
    const data = await chrome.storage.local.get(['captureCount', 'reviewDismissed']);
...
function showReviewPrompt() {
  ...
  banner.style.cssText = 'margin-top:8px;flex-direction:column;align-items:flex-start;gap:6px;';
  banner.innerHTML = `
...
function setError(error: string) {
  ...
  const isRestricted = error.includes('internal page') || error.includes('Cannot access') || error.includes('Cannot capture');
...
async function startCapture() {
  ...
  try {
    ...
    // captureVisibleTab only supports png/jpeg — capture as png, convert to pdf in editor
    const captureFormat = (selectedFormat === 'pdf' ? 'png' : selectedFormat) as ImageFormat;
    ...
    if (msg.includes('Cannot access') || msg.includes('Cannot capture') ||
        msg.includes('extension page') || msg.includes('is not allowed')) {
      setError('Cannot capture this page. Chrome blocks screenshots on internal pages. Open a regular website and try again.');
    } else {
      setError(msg);
    }
  }
...
// === Quick Actions ===
actionAgain.addEventListener('click', () => { hideStatus(); quickActions.style.display = 'none'; startCapture(); });
...
actionCopy.addEventListener('click', async () => {
  try {
    ...
    setTimeout(() => { actionCopy.innerHTML = `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="5" y="5" width="8" height="8" rx="1"/><path d="M3 11V3a1 1 0 011-1h8"/></svg> Copy`; }, 1500);
  } catch { /* no clipboard */ }
});

actionOpen.addEventListener('click', () => chrome.downloads.showDefaultFolder());
